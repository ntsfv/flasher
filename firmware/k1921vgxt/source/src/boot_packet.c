...
packet_fifo_write(uint8_t data) {
  if (!packet_fifo.full) {
...
void packet_receive(Packet_TypeDef *rx_packet) {
  ...
  // if everything is correct, then we start counting crc
    ...
    // pass what we have parsed to the kernel, the crc will be counted and
    // checked already there in the handler of a specific command
...
void packet_write_wrapper(uint8_t data) { packet_fifo_write(data); }

__attribute__((section(".ramfunc"))) void UART_IRQHandler() {
  UART->ICR = UART_ICR_RXIC_Msk;
  while (!UART->FR_bit.RXFE) {
    GPIOB->DATAOUTTGL = 1 << 3;
