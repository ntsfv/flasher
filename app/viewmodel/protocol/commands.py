...
from utils.utils import whoami, read_bin, save_bin
...
class WritePortRunnable(BaseCommandRunnable):
    ...
    def _run(self):
        ...
        self.logger.debug("Прочитано %0d байт данных из файла %s" % (len(raw_data), self.kwargs['filepath']))
        self.logger.info("Дополнение бинарного файла до размера целой страницы ...")
        data = [0xFF] * (self.kwargs['lastpage'] - self.kwargs['firstpage'] + 1) * page_size
        ...
        self.logger.info("Запись страниц%s:" % (" c предварительным стиранием" if self.kwargs['erpages'] else ""))
        for p in range(0, self.kwargs['lastpage'] - self.kwargs['firstpage'] + 1):
            cmd.cmd_write_page(self.kwargs['firstpage'] + p, data[p * page_size:p * page_size + page_size], flash, region,
                               self.kwargs['erpages'])
