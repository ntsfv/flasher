...
from utils.utils import text2int, whoami
...
class MainWindowViewModel(QMainWindow, Loggable):
    def __init__(self):
        ...
        self.mcu = mcu.get_by_name("k1921kx")
        ...
        self.ui.tedit_log.setContextMenuPolicy(
        ...
        self.ui.tinfo_tbl_flash.horizontalHeader().setSectionResizeMode(
            0, QHeaderView.Fixed
        )
        self.ui.tinfo_tbl_flash.horizontalHeader().setSectionResizeMode(
            1, QHeaderView.Fixed
        ...
        self.ui.tinfo_tbl_flash.horizontalHeaderItem(1).setToolTip(
            "Начальный адрес страницы"
        )
        self.ui.tinfo_tbl_flash.horizontalHeaderItem(2).setToolTip(
            "Размер страницы в байтах"
        )
        self.ui.tinfo_tbl_flash.horizontalHeaderItem(3).setToolTip(
            "Защита страницы от чтения"
        )
        self.ui.tinfo_tbl_flash.horizontalHeaderItem(4).setToolTip(
        ...
        self.command_senders = [self.ui.btn_connect, self.ui.btn_exec, self.ui.btn_jump]
    ...
    def _protocol_deinit(self):
        ...
        return {"pbar_callback": self.pbar_set, "is_riscv": self.is_riscv_arch()}
    ...
    def _protocol_set_cfg_word(self, **kwargs):
        ...
        return {"pbar_callback": self.pbar_set, "cfgword": cfgword, **kwargs}
    ...
    def handle_ledit_addr_edited(self):
        ...
        ledit_addr = (
            self.sender()
            .parent()
            .findChildren(QLineEdit, QtCore.QRegExp("^.*_addr$"))[0]
        )
        ledit_page = (
            self.sender()
            .parent()
            .findChildren(QLineEdit, QtCore.QRegExp("^.*_page$"))[0]
        )
        ...
        bootloader_end = (
            base_address + self.current_controller_flash()["bootflash_end_address"]
        )

        if base_address > addr or addr >= (page_size * pages) + base_address:
            self.logger.warning(
                "Адрес выходит за границы диапазона 0x%08X-0x%08X"
                % (base_address, base_address + (page_size * pages) - 1)
            )
            addr = bootloader_end

        if (
            self.get_curr_region() != "region_nvr"
            and "bootflash_end_address" in self.current_controller_flash()
            and addr < bootloader_end
        ):
        ...
        if addr != aligned_addr:
            self.logger.info(
                "Адрес 0x%08X был выровнен по размеру страницы (0x%X) - 0x%08X"
        ...
        ledit_page.setText(
            "%d" % ((aligned_addr - self.mcu.flash_base_address) // page_size)
        )
    ...
    def handle_ledit_size_edited(self):
        ...
        ledit_size = (
            self.sender()
            .parent()
            .findChildren(QLineEdit, QtCore.QRegExp("^.*_size$"))[0]
        )
        ledit_pages = (
            self.sender()
            .parent()
            .findChildren(QLineEdit, QtCore.QRegExp("^.*_pages$"))[0]
        )
        ...
        if size > ((page_size * pages_total) - addr + base_address):
            self.logger.warning(
                "Размер области должен быть не более 0x%X байт"
                % ((page_size * pages_total) - addr + base_address)
            )
            size = 0

        aligned_size = (
            (size // page_size) + (1 if size % page_size else 0)
        ) * page_size
        ledit_size.setText(
            "%s"
        ...
        if size != aligned_size:
            self.logger.info(
                "Размер 0x%X был выровнен по размеру страницы (0x%X) - 0x%X"
    ...
    def handle_ledit_pages_edited(self):
        ...
        if pages > (pages_total - page):
            self.logger.warning(
                "Указанное количество страниц больше максимального (%d)"
                % (pages_total - page)
            )
            pages = 0
    ...
    def handle_ledit_filepath_changed(self, text):
        ...
        if self.sender().path_for_open:
            if (
                os.path.isfile(self.sender().text())
                and self.sender().text()[-4:] == ".bin"
                and self.sender().text() != self.sender().last_text
            ):
        ...
        if "twrite" in self.sender().objectName() and self.is_valid_path(
            self.ui.twrite_ledit_filepath
        ):
            filesize = os.path.getsize(self.ui.twrite_ledit_filepath.text())
            page_size = self.current_controller_flash()[
                self.get_curr_region()
            ].page_size
            addr, addr_format = text2int(self.ui.twrite_ledit_addr)
            pages_total = self.current_controller_flash()[self.get_curr_region()].pages
            if filesize > (
                (page_size * pages_total) - addr + self.mcu.flash_base_address
            ):
                self.ui.twrite_ledit_size.setText("ошибка")
                self.ui.twrite_ledit_pages.setText("ошибка")
            else:
                self.ui.twrite_ledit_size.setText(
                    "0x%08X"
                    % (
                        ((filesize // page_size) + (1 if filesize % page_size else 0))
                        * page_size
                    )
                )
    ...
    def handle_tconfig_mode_select_toggled(self, state):
        self.logger.debug("Handler <%s> called" % (whoami() + "(%d)" % state))
        if state:
            for child in self.ui.tconfig_widget_cfg.findChildren(
                QCheckBox, QtCore.QRegExp(".*")
            ):
                child.setEnabled(self.ui.tconfig_rbtn_write.isChecked())
            for child in self.ui.tconfig_widget_cfg.findChildren(
                QComboBox, QtCore.QRegExp(".*")
            ):
                child.setEnabled(self.ui.tconfig_rbtn_write.isChecked())
    ...
    def handle_firstpage_select_changed(self, num):
        ...
        if num != -1:
            ...
            combo_lastpage = (
                self.sender().parent().findChildren(QComboBox, rexp_combo)[0]
            )
    ...
    def is_valid_path(self, ledit_path):
        return (
            False
            if ("red" in ledit_path.styleSheet()) or (not ledit_path.text())
            else True
        )
    ...
    def upd_flash_selected(self):
        ...
        for r in reversed(range(table.rowCount())):
            table.removeRow(r)
        pages = (
            self.current_controller_flash()[self.get_curr_region()].pages
            - self.current_controller_flash()[self.get_curr_start_page()]
        )
        page_size = self.current_controller_flash()[self.get_curr_region()].page_size
        ...
        start_address = (
            self.current_controller_flash()[self.get_curr_start_page()]
            if self.get_curr_start_page() in self.current_controller_flash()
            else 0
        )
    ...
    def upd_twrite_jumpaddr(self, state: bool):
        ...
        # self.ui.twrite_ledit_jumpaddr.setEnabled(state)
        self.ui.twrite_ledit_jumpaddr.setText(
            "0x%08X"
            % (
                self.mcu.flash_base_address
                + self.current_controller_flash()["bootflash_end_address"]
            )
        )
    ...
    def update_address_fields(self):
        for address_field in self.address_fields:
            address_field.setText(
                ...
                % (
                    0
                    if self.get_curr_region() == "region_nvr"
                    else self.mcu.flash_base_address
                    + self.current_controller_flash()["bootflash_end_address"]
                )
            )
    ...
    def exec_tab_write(self):
        ...
        self.logger.info(
            "Область - %s"
            % ("основная" if self.get_curr_region() == "region_main" else "NVR/Info")
        )
        ...
        if self.ui.twrite_ledit_size.text() == "ошибка":
            self.logger.error(
                "Не выполнено - размер файла превышает размер выбранной области!"
            )
            return
        else:
        ...
        if filevalid:
            self.logger.info(
                'Файл - "%s", размер %d байт' % (filepath, os.path.getsize(filepath))
            )
        else:
        ...
        for p in range(firstpage, lastpage + 1):
            if curr_flash.wr_lock[p]:
                ...
                self.logger.error(
                    "Не выполнено - одна или несколько модифицируемых страниц защищены от записи/стирания"
                )
                return
        ...
        self._protocol_write(
            ...
            jump=jump,
            jumpaddr=jumpaddr,
            current_region=self.get_curr_region(),
            current_flash=self.get_curr_flash(),
        )
    ...
    def exec_tab_erase(self):
        ...
        if erall:
            ...
            if pages:
                ...
                self.logger.error(
                    "Не выполнено - не определён размер стираемой области"
                )
                return
        else:
            self.logger.error("Не выполнено - режим стирания не определён")
        ...
        self.logger.info(
            "Модифицируемые страницы - %d ... %d (%d байт)"
    ...
    def exec_tab_read(self):
        ...
        for p in range(firstpage, lastpage + 1):
            if curr_flash.rd_lock[p]:
                self.logger.error(
                    "Не выполнено - одна или несколько считываемых страниц защищены от чтения"
                )
                return
